...
<img src="https://raw.githubusercontent.com/prajwalpr4/prajwalpr4/main/dist/profile-details.svg" width="100%" />

<br><br>

<img src="https://raw.githubusercontent.com/prajwalpr4/prajwalpr4/main/dist/most-commit-language.svg" width="100%" />

</div>

> ⚠️ **These cards need a one-time GitHub Action setup**, same as the snake below — they're generated by a separate open-source project (`github-profile-summary-cards`) and can't be pulled from a hosted API like the badges above.

**One-time setup (takes 2 minutes):**

1. Create the workflow file `.github/workflows/profile-summary.yml` in **this exact repo** (`prajwalpr4/prajwalpr4`) with the content below
2. Go to **Settings → Actions → General → Workflow permissions** → select **Read and write permissions** → Save
3. Go to the **Actions** tab → select **Generate GitHub Analytics Cards** → click **Run workflow** (manually trigger it once)
4. It will generate SVG cards into a `dist/` folder and commit them to your `main` branch automatically
5. Refresh this README — the cards will now render and will auto-update daily after that

```yaml
name: Generate GitHub Analytics Cards
  ...
  build:
    ...
    steps:
      - name: Generate profile summary cards
        uses: vn7n24fzau6q/github-profile-summary-cards@release
        with:
...
> 💡 The action ships several themes (e.g. `github_dark`, `algolia`, `monokai`). None are pure green out of the box — check the [project's theme gallery](https://github.com/vn7n24fzau6q/github-profile-summary-cards#-themes) and swap the `theme:` value if you want a closer color match. `github_dark` blends best with the rest of this README's palette.
